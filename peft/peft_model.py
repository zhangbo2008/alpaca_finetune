    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            self._setup_prompt_encoder()#一种是prompt学习,一种是lora
        else:
...
#=设置word_embeddings层.
    ...
    def get_prompt(self, batch_size):
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            else:#对我们前面添加的prompt: 0--->20 进行编码. 进行学习. 让他进行finetune
...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            past_key_values = self.get_prompt(batch_size)# prefix_tunning模式就是在模型起那么加入一个prompt工程的层. 然后我们把这个当做past_key_values传入. 因为他是prompt. 他等于在我们之前进行提供上下问信息.所以他就是基础模型对应的past_key_values.
            return self.base_model(
